...
def test_l2_project(k):
    ...
    l2_error = mesh.mpi_comm().allreduce(
        assemble_scalar(dot(u - v, u - v) * dx), op=MPI.SUM
    )
    assert l2_error < 1e-15


@pytest.mark.parametrize("k, lb, ub", [(1, -3.0, -1.0), (2, -3.0, -1.0)])
...
def test_l2_project_bounded(k, lb, ub):
    ...
    assert np.min(np.asarray(vh.x.array)) < ub + 1e-12
    assert np.max(np.asarray(vh.x.array)) > lb - 1e-12
